...
HRESULT CParticle::Init(int nTex,int nCount)
{
	if (m_type == TYPE::TYPE_NORMAL)
	{
		m_Info.move = D3DXVECTOR3(
			sinf((float)(rand() % PI - HARF_PI) / MAGNI) * (float)(rand() % (int)m_move.x - m_move.x * 0.5f),	//xの移動量
			cosf((float)(rand() % PI - HARF_PI) / MAGNI) * (float)(rand() % (int)m_move.y - m_move.y * 0.5f),	//yの移動量
			INITD3DXVECTOR3.z);
	}
...
void CParticle::Update(void)
{
	...
	//割合計算
	float fCountRate = RNLib::Ease().Easing(EASE_TYPE::OUT_SINE, m_Info.nCount, m_Info.nCountMax);
