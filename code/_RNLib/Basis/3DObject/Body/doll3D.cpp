	...
	if (drawVtxIdxNum > 0) {
		...
		for (int cntVtx = drawVtxIdxNum - 1; cntVtx >= 0; cntVtx--) {
			...
			RNLib::Polygon3D().Put((UShort)RNMode::PRIORITY::UI3D, vtxInfo[vtxIdxs[cntVtx]].worldPos, INITROT3D)
				...
				->SetZTest(false)
				->SetBillboard(true);
			RNLib::Text3D().Put((UShort)RNMode::PRIORITY::UI3D, String("%d", vtxIdxs[cntVtx]), _RNC_Text::ALIGNMENT::CENTER, NONEDATA, vtxInfo[vtxIdxs[cntVtx]].worldPos, INITROT3D)
		...
		for (int cntVtx = drawVtxIdxNum - 1; cntVtx >= 0; cntVtx--) {
			...
			RNLib::Polygon3D().Put((UShort)RNMode::PRIORITY::UI3D, vtxInfo[cntVtx].worldPos, INITROT3D)
				...
				->SetZTest(false)
				->SetBillboard(true);
			RNLib::Text3D().Put((UShort)RNMode::PRIORITY::UI3D, String("%d", cntVtx), _RNC_Text::ALIGNMENT::CENTER, NONEDATA, vtxInfo[cntVtx].worldPos, INITROT3D)
				->SetSize(Size2D(0.5f, 0.5f))
				->SetLighting(false)
				->SetZTest(false)
				->SetBillboard(true)
